...
function renderGrid() {
  ...
  tickets.forEach(t => {
    ...
    item.innerHTML = `<div class="qr-box" id="qr-${t.id}"></div><div style="margin-top:8px">${t.id}</div>`;
    qrcodesEl.appendChild(item);

    const qrElement = item.querySelector(".qr-box");
    const ok = renderTicketQRCode(qrElement, t.id);
    if (!ok) {
      item.classList.add("qr-error-state");
      renderErrors.push(t.id);
    }
  });

  updateQrStatusBanner(renderErrors);
}

function renderTicketQRCode(targetEl, ticketId) {
  if (!targetEl) return false;
  targetEl.innerHTML = "";

  if (typeof QRCode === "undefined") {
    targetEl.innerHTML = `<small class="qr-error">QR library missing</small>`;
    return false;
  }

  try {
    new QRCode(targetEl, { text: ticketId, width: 100, height: 100, margin: 1 });
  } catch (err) {
    console.error("QRCode creation failed:", err);
    targetEl.innerHTML = `<small class="qr-error">QR error: ${err.message}</small>`;
    return false;
  }

  if (!hasQrOutput(targetEl)) {
    targetEl.innerHTML = `<small class="qr-error">QR failed to render</small>`;
    return false;
  }
...
function hasQrOutput(container) {
  if (!container) return false;
  return Boolean(container.querySelector("canvas") || container.querySelector("img") || container.querySelector("svg"));
}

function getQrStatusBanner() {
  let banner = document.getElementById("qrStatusBanner");
  if (!banner) {
    banner = document.createElement("div");
    banner.id = "qrStatusBanner";
    banner.className = "qr-status-banner hidden";
    const parent = qrcodesEl ? qrcodesEl.parentElement : null;
    if (parent) {
      parent.insertBefore(banner, qrcodesEl);
    }
  }
  return banner;
}

function updateQrStatusBanner(errors = []) {
  const banner = getQrStatusBanner();
  if (!banner) return;
  ...
  banner.innerHTML = `⚠️ Unable to render ${errors.length} QR code${plural ? "s" : ""}. IDs: <strong>${errors.join(", ")}</strong>`;
...
const FEEDBACK_TIMING = {
  success: { hideMs: 2200, cooldownMs: 1000 },
  warning: { hideMs: 3300, cooldownMs: 3000 },
  error: { hideMs: 3300, cooldownMs: 3000 }
};
let scanCooldownTimer = null;
let feedbackHideTimer = null;
let feedbackTapHandler = null;

function hideScanFeedback(fromTap = false) {
  if (!scanFeedbackEl) {
    clearFeedbackTimers();
    releaseScanner();
    return;
  }
  if (!scanFeedbackEl.classList.contains("hidden")) {
...
function startFeedbackLifecycle(type, overrides = {}) {
  if (!scanFeedbackEl) return;
  const base = FEEDBACK_TIMING[type] || FEEDBACK_TIMING.success;
  const timing = { ...base, ...overrides };
  ...
  feedbackTapHandler = () => hideScanFeedback(true);
